...
contract OceanKeeper is Context, IBEP20, Ownable {
    ...
    uint256 public _TAXATION_COST;
    uint256 public _BURN_COST;
    uint256 public _CHARITY_COST;

    // Track INITIALinal fees to bypass fees for charity account
    uint256 private INITIAL_TAXATION_COST;
    uint256 private INITIAL_BURN_COST;
    uint256 private INITIAL_CHARITY_COST;

    constructor (address _DevFeeAddress,address tokenOwner) {
		_NAME = "Ocean Keeper";
		_SYMBOL = "$OCK";
		_DECIMALS = 18;
		...
		_TAXATION_COST = 3 * 100; 
        _BURN_COST = 3 * 100;
		_CHARITY_COST = 4 * 100;
		INITIAL_TAXATION_COST = 3;
		INITIAL_BURN_COST = 3;
		INITIAL_CHARITY_COST = 4;
		_isCharity[_DevFeeAddress] = true;
		FeeAddress = _DevFeeAddress;
		_owner = tokenOwner;
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee) onlyOwner() public{
        _TAXATION_COST = _txFee* 100; 
        _BURN_COST = _burnFee * 100;
		_CHARITY_COST = _charityFee* 100;
		INITIAL_TAXATION_COST = _TAXATION_COST;
		INITIAL_BURN_COST = _BURN_COST;
		INITIAL_CHARITY_COST = _CHARITY_COST;
	}
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, _TAXATION_COST, _BURN_COST, _CHARITY_COST);
    ...
    function removeAllFee() private {
        if(_TAXATION_COST == 0 && _BURN_COST == 0 && _CHARITY_COST == 0) return;
    ...
    function restoreAllFee() private {
        _TAXATION_COST = INITIAL_TAXATION_COST;
        _BURN_COST = INITIAL_BURN_COST;
        _CHARITY_COST = INITIAL_CHARITY_COST;
    }
    
    function _getTaxFee() private view returns(uint256) {
        return _TAXATION_COST;
    }
