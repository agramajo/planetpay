...
contract PlanetPayF is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 21000 * 10**4 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "PlanetPayF";
    string private _symbol = "PlanetPayF";
    ...
    // devFee is for project development and marketing
    ...
    address public _devAddress = 0x2E9027F3C5871dB187A13fC14e0766F81aF58d37;
    address public _charityAddress = 0x306547ED03d4D914544a7B19E88cA69C75C2A20a;  
    address public _liquidityTaxAddress = 0x44E6FfA1f13Aba4EfEDFf3E848A3Db70154d700C; 
    ...
    uint256 public _maxTxAmount = 1000 * 10**4 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 1000 * 10**4 * 10**9;
    ...
    constructor () public {
        ...
        //exclude owner (deployer) and this contract from fee
    ...
    enum ActionType {
        ...
        setCharityFeePercent,    // 7
        setDevFeePercent,  // 8
        setdevAddress,     // 9
        setCharityAddress,       // 10
        setMaxTxPercent,         // 11
        setSwapAndLiquifyEnabled,// 12
        setLiquidityTaxAddress,  // 13
        withdrawFromCharity,     // 14
        withdrawFromDev,   // 15
        lock,                    // 16
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require((taxFee + _liquidityFee + _charityFee + _devFee) <= 10, "Fee needs to be in allowable range");
            _taxFee = taxFee;
        
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require((_taxFee + liquidityFee + _charityFee + _devFee) <= 10, "Fee needs to be in allowable range");
    ...
    function setCharityFeePercent(uint256 charityFee) external onlyOwner() {
        require((_taxFee + _liquidityFee + charityFee + _devFee) <= 10, "Fee needs to be in allowable range");
            _charityFee = charityFee;
        
    }

    function setDevFeePercent(uint256 devFee) external onlyOwner() {
        require((_taxFee + _liquidityFee + _charityFee + devFee) <= 10, "Fee needs to be in allowable range");
            _devFee = devFee;
    ...
    function withdrawFromCharity(address _sendTo, uint256 _amount) public onlyOwner {
            _tokenTransfer(_charityAddress, _sendTo, _amount, false);
        
        
    }
    
    function withdrawFromDev(address _sendTo, uint256 _amount) public onlyOwner {
        
            _tokenTransfer(_devAddress, _sendTo, _amount, false);
    ...
    // Add additional devFee  parameter from _getTValues
    // Pass dev fee returned into _getRValues
    // Return tdev fee
    ...
    //* Just add calculatedevFee function
    //* Subtract from tamount to give tTransferAmount
    //* Return extra dev fee amount (additional param, alter calls) 
    ...
    //* pass in dev fee
    //* Just add line to calculate dev rate
    //* Subtract from Total
    ...
    function calculateFeeToTake(uint256 _amount) private view returns (uint256) {
        uint256 feeToTake = _devFee.add(_charityFee).add(_liquidityFee);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _charityFee == 0 && _devFee == 0 ) return;
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, team, charity, liquidity fee
        _tokenTransfer(from,to,amount,takeFee);
    }
    
    function _getFeeAmounts(uint256 amount) private view returns (uint256, uint256, uint256) {
        
        uint256 totalFee = _devFee.add(_charityFee).add(_liquidityFee);        
        uint256 liquidityAmount;        
        uint256 devAmount = amount.mul(_devFee).div(totalFee);
        uint256 charityAmount = amount.mul(_charityFee).div(totalFee);
        uint256 threeFeeAmount = devAmount.add(charityAmount);
        ...
        return (devAmount, charityAmount, liquidityAmount);
    }

    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {

        (uint256 devAmount, uint256 charityAmount, uint256 liquidityAmount) = _getFeeAmounts(contractTokenBalance);
        
        // Send to dev addie
        if(devAmount > 0){
            _tokenTransfer(address(this), _devAddress, devAmount, false);
        }
