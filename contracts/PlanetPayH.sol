...
contract PlanetPayH is Context, IERC20, Ownable, ReentrancyGuard {
	...
	address CHARITY_ADDRESS = 0xc0d935CD0a32B76Aed3607Caedba5172cf3042E3;
	address MARKETING_ADDRESS = 0xc0516A5Ef21Fa607f6c733B3bFa10FDAAf219c1B;

	uint256 private constant MAX = ~uint256(0); // 210M
	uint256 private _tTotal = 210000000 * 10**9;
	uint256 private _rTotal = (MAX - (MAX % _tTotal));
	uint256 private _tHODLrRewardsTotal;

	string private _name = "PlanetPayH";
	string private _symbol = "PlanetPayH";
	...
	function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
		(uint256 tTransferAmount, uint256 tHODLrFee, uint256 tCharity, uint256 tMarketing) = _getTValues(tAmount);
		(uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee) = _getRValues(tAmount, tHODLrFee, tCharity, tMarketing, _getRate());
		return (rAmount, rTransferAmount, rHODLrFee, tTransferAmount, tHODLrFee, tCharity, tMarketing);
	}
	...
	function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
		...
		uint256 tTransferAmount = tAmount.sub(tHODLrFee).sub(tCharity).sub(tMarketing);
		return (tTransferAmount, tHODLrFee, tCharity, tMarketing);
	}

	function _getRValues(uint256 tAmount, uint256 tHODLrFee, uint256 tCharity, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
		uint256 rAmount = tAmount.mul(currentRate);
		uint256 rHODLrFee = tHODLrFee.mul(currentRate);
		uint256 rCharity = tCharity.mul(currentRate);
		uint256 rMarketing = tMarketing.mul(currentRate);
		uint256 rTransferAmount = rAmount.sub(rHODLrFee).sub(rCharity).sub(rMarketing);
	...
	function removeAllFee() private {
		if(_rewardFee == 0 && _charityFee == 0 && _marketingFee == 0) return;		
	...
	function _transferMarketing(uint256 tMarketing) private {
		...
		if(_isExcludedFromReward[MARKETING_ADDRESS])
